...
nav.on('pop', ({ removed }) =>
  removed.forEach((e) => e.el.dispatchEvent(new Event('sn:destroyed'))),
);
...
const TOPICS = [
  [
    'Push',
    "The incoming page slides in from the trailing edge over 500 ms on cubic-bezier(0.32, 0.72, 0, 1). The page beneath slides 30 % of the width toward the leading edge and dims to 10 % black. A soft shadow rides the incoming page's leading edge so the two read as stacked.",
  ],
  [
    'Who animates',
    "Not this script. The engine writes where each page should end up and puts that phase's duration and curve in --sn-t and --sn-e on the container; the frames in between belong to CSS, and to the compositor. A whole 500 ms push costs about a dozen style writes, so a busy main thread cannot stutter it. The travel is a share of the page rather than a pixel count, so nothing measures layout and a resize mid-transition stays honest. Which way forward is comes from --sn-dir, flipped for right-to-left.",
  ],
  [
    'Pop',
    'The exact reverse. Because both movements are expressed as one number, p, how much of the upper page is showing, pop is push run backwards and nothing is duplicated.',
  ],
  [
    'Interactive pop',
    'beginInteractivePop() lets an app that owns the edge -- an installed PWA, a native webview -- set p directly from the finger: p = 1 − dx / width. While the finger is down --sn-t is 0s, so nothing is animated; the page is simply where the finger says it is. The library ships no recognizer of its own: in a browser tab the browser already owns the edge.',
  ],
  [
    'Release',
    'finish({ complete }) settles the rest of the way. Past half the width, or a flick faster than 500 px/s toward the trailing edge, an app would complete. A flick back faster than 500 px/s cancels. Otherwise it snaps back. The remaining distance runs an ease-out whose duration comes from distance ÷ velocity, clamped to 120–400 ms, so a fast flick finishes fast and a slow release finishes slow. That number is handed to CSS and the browser runs it out.',
  ],
  [
    'Scroll',
    "Pages beneath the top stay in the DOM, hidden. The transition writes only transform, so a page's scroll offset, form state and focus are untouched when you come back to it. No restoration logic exists because none is needed.",
  ],
  [
    'Your chrome',
    'A tab bar beside the pages can read --sn-t and --sn-e and the sn-page-upper / sn-page-lower classes and move in lockstep with the pages, on the compositor, from CSS alone. Inside a page the two properties are pinned, so that a phase starting never re-resolves the style of every row of every kept page; a page that wants them lifts that with one rule. For chrome that needs the number, the stack still emits progress events with (lower, upper, p) — and only runs a frame loop while something is listening.',
  ],
  [
    'Tuning',
    'The look is a set of CSS custom properties on the container: --sn-duration, --sn-easing, --sn-parallax, --sn-dim-color, --sn-dim-max, --sn-shadow, the three --sn-settle-* knobs and --sn-time-scale. They are read when a transition starts, so a media query, a theme class or one inline style is enough to slow the animation down or soften it. Unset ones keep the defaults. The Options page below changes nothing but CSS.',
  ],
  [
    'History',
    'An optional adapter mirrors depth into history.state, so the browser or hardware back button pops with the same animation. On iOS browsers the pop is instant, because Safari already animated its own snapshot.',
  ],
];
    ...
    body: (b) => {
      b.append(
        h(
          'p',
          'lede',
          "A drop-in native push/pop transition for any web app: the iOS look on iOS and elsewhere, the Android look on Android. Pages, headers and styling are the app's own; the engine only moves them.",
        ),
      );
      ...
      b.append(
        link('Deep stack', () => nav.push(depthPage(2)), 'push, push, push, then swipe back'),
      );
    ...
    body: (b) => {
      b.append(
        h(
          'p',
          'lede',
          `This is level ${n} of the stack. Each level keeps its own scroll offset while it sits underneath.`,
        ),
      );
  ...
  page({
    ...
    body: (b) => {
      ...
      for (const [value, label] of [
        ['browser', 'Browser — use browser defaults'],
        ['disabled', 'Disabled — suppress swipes where supported'],
      ]) {
      ...
      b.append(
        ...
        h(
          'p',
          'note',
          'The library has no back gesture of its own: in a browser tab the browser owns the edge, and a second recognizer reads as two backs at once. Disabled asks the browser to suppress its gesture, which depends on the browser and OS. Back buttons work in both modes.',
        ),
      );
      ...
      b.append(
        h(
          'p',
          'note',
          'Sets --sn-time-scale: 4 on the container, multiplying every duration the engine hands to CSS, so the parallax, dim and settle curve are easy to watch.',
        ),
      );
      b.append(
      ...
      b.append(
        h(
          'p',
          'note',
          'A class on the container that shortens --sn-duration, flattens --sn-parallax and lightens the dim and shadow. Pure CSS: the engine is not reconfigured.',
        ),
      );
    },
  });

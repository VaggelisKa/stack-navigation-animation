// Shared setup for the e2e suite: serves dist/, launches the preinstalled
// Chromium through playwright-core, and reads the outlet's DOM.
...
export async function launch({ width = 420, height = 800 } = {}) {
  ...
  /** Wait one frame before reading, so a click's render has landed. */
  ...
  const state = async () =>
    ...
    page.evaluate(() => {
      ...
      const name = (p) => p.querySelector('[data-page]')?.dataset.page ?? '?';
      return {
        url: location.pathname + location.search,
        busy: outlet.classList.contains('sn-busy'),
        pages: pages.map(name),
        visible: pages.filter((p) => p.classList.contains('sn-page-visible')).map(name),
        title: pages.at(-1)?.querySelector('h1')?.textContent?.trim(),
      };
    }));
  const busy = (timeout = 2000) => page.waitForFunction(() => document.querySelector('.outlet').classList.contains('sn-busy'), null, { timeout }).catch(() => {});
  const settled = () => page.waitForFunction(() => !document.querySelector('.outlet').classList.contains('sn-busy'));
  ...
  const top = () => page.locator('.outlet > .sn-page-visible').last();
  const scrollTop = () => page.evaluate(() => document.querySelector('.outlet > .sn-page-visible').scrollTop);
  const setScroll = (y) => page.evaluate((y) => (document.querySelector('.outlet > .sn-page-visible').scrollTop = y), y);
  ...
  const swipeBack = async ({ until = 0.8, y: yFrac = 0.5, mid } = {}) => {
    ...
    // Playwright moves the mouse at flick speed. A drag that should cancel has
    // to come to rest before it is released, or the velocity completes it.
    if (until < 0.5) {
      await page.waitForTimeout(200);
      await page.mouse.move(box.x + x - 30 + 1, y);

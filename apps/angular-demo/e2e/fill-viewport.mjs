...
const fits = async (label) => {
  await page.waitForFunction(() => {
    ...
    return Math.abs(rect.bottom - (viewport ? viewport.offsetTop + viewport.height : innerHeight)) < 1;
  });
  check(true, label);
};
await fits('auto-height slot fills viewport below shell header');
eq(await page.locator('.embedded-stack').evaluate((el) => el.clientHeight), 720, '80px header leaves 720px');
...
await page.waitForFunction(() => document.querySelector('.shell-header').classList.contains('expanded'));
await fits('follows header expansion without a window resize');
eq(await page.locator('.embedded-stack').evaluate((el) => el.clientHeight), 660, 'expanded header leaves 660px');
...
const styleWrites = await page.evaluate(async () => {
  ...
  observer.observe(document.querySelector('.embedded-stack'), { attributes: true, attributeFilter: ['style'] });
...
await page.locator('.microfrontend').evaluate((el) => { el.style.transform = 'translateY(25px)'; });
await fits('follows position-only layout changes');
await page.locator('.microfrontend').evaluate((el) => { el.style.transform = ''; });
...
await page.evaluate(() => Object.defineProperty(window, 'visualViewport', {
  configurable: true, value: { offsetTop: 30, height: 500 },
}));
await fits('uses visible viewport height and offset (keyboard/pan simulation)');
eq(await page.locator('.embedded-stack').evaluate((el) => el.getBoundingClientRect().bottom), 530, 'visible bottom includes offset');
await page.evaluate(() => Object.defineProperty(window, 'visualViewport', { configurable: true, value: null }));
await fits('falls back to innerHeight when VisualViewport is unavailable');
await page.evaluate(() => delete window.visualViewport);

await page.locator('.shell-header').evaluate((el) => { el.style.height = '1200px'; });
...
await page.locator('.shell-header').evaluate((el) => { el.style.height = ''; });
await fits('recovers from a zero-height slot');

await page.locator('.microfrontend').evaluate((el) => { el.style.display = 'none'; });
await page.setViewportSize({ width: 420, height: 800 });
await page.locator('.microfrontend').evaluate((el) => { el.style.display = ''; });
...
await page.evaluate(() => {
  ...
  window.removedStack.getClientRects = () => { window.stackReads++; return read(); };
...
eq(await page.evaluate(() => window.removedStack.style.height), '123px', 'teardown restores original height');
eq(await page.evaluate(() => window.removedStack.style.boxSizing), 'content-box', 'teardown restores original box sizing');
eq(await page.evaluate(() => window.removedStack.style.getPropertyPriority('height')), '', 'teardown restores original priority');
...
await page.waitForFunction(() => document.querySelector('iframe').contentDocument.querySelector('.embedded-stack')?.clientHeight === 420);
eq(await embedded.locator('.embedded-stack').evaluate((el) => el.clientHeight), 420, 'iframe uses its own 500px viewport');

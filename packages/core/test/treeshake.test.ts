...
const ALL = ['animate', 'css-vars', 'direction', 'edge-pan-gesture', 'history-adapter', 'index', 'native-transition', 'navigation-stack', 'platform', 'styles'];
...
test('createNativeStack leaves out direction resolution, the history adapter and the stylesheet', async () => {
  assert.deepEqual(await survivors(`export { createNativeStack } from './index.ts';`), ['animate', 'css-vars', 'edge-pan-gesture', 'index', 'native-transition', 'navigation-stack', 'platform']);
});
